...
PackageScoped[{
  planStructuralIR, executeExecutionPlan, renderExecutionPlan,
  tryStructuralIRPlan
}]
...
planStructuralIR[solved : irp["SolvedDesc"][a_Association], operator_] :=
  Module[{normalized, na, inputs, outputs, axisSizes, inShapes, outShapes,
    ...
    If[Length[inShapes] =!= 1 || Length[outShapes] =!= 1,
      Return[plannerFailure["StructuralArity", <|
        "Inputs" -> Length[inShapes], "Outputs" -> Length[outShapes]|>]]];
    ...
    If[plannerFailureQ[inAtoms], Return[inAtoms]];
    If[plannerFailureQ[outAtoms], Return[outAtoms]];
    inKeys = planAtomKey /@ inAtoms;
    outKeys = planAtomKey /@ outAtoms;
    If[! DuplicateFreeQ[inKeys],
      Return[plannerFailure["RepeatedInputAtom", <|"Keys" -> inKeys|>]]];
    If[! DuplicateFreeQ[outKeys],
      Return[plannerFailure["DuplicateOutputAtom", <|"Keys" -> outKeys|>]]];
    ...
    If[violations =!= {},
      Return[plannerFailure["DroppedNonUnitAtom", <|"Atoms" -> violations|>]]];
    ...
    broadcastResult = Catch[
      ...
      Null,
      planBroadcastTag];
    If[plannerFailureQ[broadcastResult], Return[broadcastResult]];
...
executeExecutionPlan[irp["ExecutionPlan"][steps_List, meta_Association],
  ...
  Catch[Module[{value, step},
    If[Length[tensors] =!= meta["InputCount"],
      Return[plannerFailure["PlanInputCount", <|
        "Expected" -> meta["InputCount"], "Actual" -> Length[tensors]|>]]];
    value = First[tensors];
    Do[
      step = st;
...
tryStructuralIRPlan[h_Hold, tensors_List, bindings_List, operator_String,
  ...
  Module[{compiled, solvedBundle, solved, analysis, plan, held},
    ...
    If[plannerFailureQ[plan], Return[Missing["UnsupportedIR"]]];
