...
`Einstoff[Operate][f][desc, tensors]` applies a shape-preserving operation `f` to targeted blocks of a single tensor.

`Einstoff[Map][f][desc, tensors]` maps a possibly shape-changing operation `f` over targeted blocks of a single tensor.
...
- `Einstoff[ArrayReduce]` allows named recipes `"sum" | "total" | "add"` `"mean" | "average"` `"max"` `"min"` `"prod" | "product" | "times"` `"var" | "variance"` `"std" | "stddev"` `"count_nonzero" | "countnonzero"` `"any"` `all` and `"logsumexp" | "lse"`;
  `Einstoff[Operate]` allows named recipes `id` `flip` `sort` `softmax` and `log_softmax`.
...
In[16]:= Einstoff[Operate]["flip"][{{a_, Slot["b"]}} :> {{a, Slot["b"]}}, {x}]

Out[16]= {{4, 3, 2, 1}, {8, 7, 6, 5}}
```

Use a Wolfram function as the operation:

```wl
In[17]:= Einstoff[Operate][Reverse][{{a_, Highlighted[b_]}} :> {{a, Highlighted[b]}}, {ArrayReshape[Range[6], {2, 3}]}]

Out[17]= {{3, 2, 1}, {6, 5, 4}}
```

Use `Map` for a shape-changing block transform:

```wl
In[18]:= Einstoff[Map][Total][{{a_, Slot["b"]}} :> {{a}}, {ArrayReshape[Range[6], {2, 3}]}]
...
```wl
In[19]:= Einstoff[Inner][Plus, Min][{{a_}, {b_}} :> {{a, b}}, {Range[2], Range[3]}]
...
```wl
In[20]:= Einstoff[Dot][{{a_, Slot["b"]}, {Slot["b"], c_}} :> {{a, c}}, {x, y}, {}, "Targeting" -> True]

Out[20]= {{38, 44, 50, 56}, {83, 98, 113, 128}}
```

Use a within-tensor pairwise contraction:

```wl
In[21]:= t = ArrayReshape[Range[16], {2, 2, 2, 2}];
In[22]:= Einstoff["ArrayContract"][{{a_, b_, a_, d_}} :> {{b, d}}, {t}]
...
```wl
In[23]:= x = ArrayReshape[Range[6], {2, 3}];
In[24]:= y = ArrayReshape[Range[8], {2, 4}];
In[25]:= Einstoff[Join][{{m_, a_}, {m_, b_}} :> {{m, a ⊕ b}}, {x, y}]

Out[25]= {{1, 2, 3, 1, 2, 3, 4}, {4, 5, 6, 5, 6, 7, 8}}
```
...
```wl
In[26]:= Einstoff[ArrayReduce]["max"][{{a_, b_}} :> {{a}}, {ArrayReshape[Range[12], {3, 4}]}]
...
```wl
In[27]:= x = ArrayReshape[Range[20], {2, 10}];
In[28]:= Einstoff[Split][{{b_, "q" ⊕ k_}} :> {{b, "q"}, {b, k}}, {x}, {"q" -> 3}]

Out[28]= {{{1, 2, 3}, {11, 12, 13}}, {{4, 5, 6, 7, 8, 9, 10}, {14, 15, 16, 17, 18, 19, 20}}}
```
...
```wl
In[29]:= x = ArrayReshape[Range[2*4*6], {2, 4, 6}];
In[30]:= Einstoff[ArrayReshape][
  {{b_, h_ ⊗ "h1", w_ ⊗ "w1"}} :>
    {{b, h, w, "h1" ⊗ "w1"}},
  {x}, {"h1" -> 2, "w1" -> 3}]

Out[30]= {{{{1, 2, 3, 7, 8, 9}, {4, 5, 6, 10, 11, 12}}, {{13, 14, 15, 19, 20, 21}, {16, 17, 18, 22, 23, 24}}}, {{{25, 26, 27, 31, 32, 33}, {28, 29, 30, 34, 35, 36}}, {{37, 38, 39, 43, 44, 45}, {40, 41, 42, 46, 47, 48}}}}
```

Global pooling:

```wl
In[31]:= x = ArrayReshape[Range[2*3*4], {2, 3, 4}];
In[32]:= Einstoff[ArrayReduce][Mean][{{batch_, height_, width_}} :> {{batch}}, {x}]
...
```wl
In[33]:= x = ArrayReshape[Range[24], {2, 3, 4}];
In[34]:= y = ArrayReshape[Range[40], {2, 4, 5}];
In[35]:= Einstoff[Dot][{{n_, a_, b_}, {n_, b_, c_}} :> {{n, a, c}}, {x, y}]

Out[35]= {{{110, 120, 130, 140, 150}, {246, 272, 298, 324, 350}, {382, 424, 466, 508, 550}}, {{1678, 1736, 1794, 1852, 1910}, {2134, 2208, 2282, 2356, 2430}, {2590, 2680, 2770, 2860, 2950}}}
```
...
In[36]:= x11 = {{0, 1}};
In[37]:= x12 = {{10, 11, 12}};
In[38]:= x21 = {{20, 21}, {22, 23}};
In[39]:= x22 = {{30, 31, 32}, {33, 34, 35}};

In[40]:= Einstoff["Massage"][
  {{a_, b_}, {a_, c_}, {d_, b_}, {d_, c_}} :>
    {{a ⊕ d, b ⊕ c}},
  {x11, x12, x21, x22}]

Out[40]= {{0, 1, 10, 11, 12}, {20, 21, 30, 31, 32}, {22, 23, 33, 34, 35}}
```
...
```wl
In[41]:= Block[{c = 3}, Einstoff[ArrayReshape][{{"c"}} :> {{"c"}}, {Range[5]}]]
...
```wl
In[42]:= Einstoff["Massage"][{{a_}} :> {{a, c}}, {Range[4]}, {c -> 3}]

Out[42]= {{1, 1, 1}, {2, 2, 2}, {3, 3, 3}, {4, 4, 4}}
```

Target heads are part of targeted binding keys:

```wl
In[43]:= Einstoff[Operate]["id"][{{a_, Slot["b"]}} :> {{a, Slot["b"]}}, {ArrayReshape[Range[6], {2, 3}]}]
...
```wl
In[44]:= Einstoff[Join][{{a_}, {b_}} :> {{a ⊕ b}}, {Range[2], Range[3]}]
